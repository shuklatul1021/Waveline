...
export default function JoinPage() {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      router.push(
        `/meeting/${data.meeting.id}?name=${encodeURIComponent(name)}&type=${data.meeting.type}`,
      );
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-[#FAFAF9] flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-bold text-[#37322F] mb-2">
            Unable to Join
          </h1>
        ...
        <div className="grid md:grid-cols-2">
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className="flex items-center gap-2 text-sm text-[#6B6B68] mb-6">
              <Users size={16} />
              <span>
                {meetingInfo?._count.participants} /{" "}
                {meetingInfo?.maxParticipants} participants
              </span>
            </div>
